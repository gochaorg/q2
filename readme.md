...
Теперь сам запрос строиться так
    ...
    foo.where(row => row.id == 1).pick(row => console.log(row))
    
При построении запроса на клиенте не выполняются действия на данными, а создаеться объект который описывает данный запрос, ниже его пример (в примере частично отображена структура)
...
Последней инструкцией в этой цепоче методов должен быть вызов `pick(...)`. Метод `pick` отправляет POST запрос на REST сервис, в теле которого передается созданный объект, соответ запросу.
